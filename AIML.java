...
public class AIML implements ActionListener {
    JFrame M = new JFrame();
    private JTextArea ta;
    private JScrollPane sp;
    private JButton btnSyllabus;
    private JButton btnStaffDetails;
    JPanel forgroundPanel = new JPanel() {
        @Override
        protected void paintComponent(Graphics g) {
            super.paintComponent(g);
            // Load and draw your image here
            ImageIcon imageIcon = new ImageIcon("D:/pictures/Anjuman/AIandML.gif");
    ...
    public AIML() {
        M.setTitle("Artificial intelligence and Machine learning");
        ...
        forgroundPanel.setLayout(null);
        forgroundPanel.setBounds(0, 0, 700, 320);
        ta.add(forgroundPanel);
    ...
    public void actionPerformed(ActionEvent e) {
        if (e.getSource() == btnSyllabus) {
            ...
            sp.setViewportView(tk); // Set the table as the view of the scroll pane
    ...
    String[][] insertTable() {
        try {
            ...
            ResultSet rs = stmt.executeQuery("SELECT * FROM Staff_detail WHERE Working_branch = 'Artificial intelligence and ML';");
    ...
    String[][] getSyllabusForSemester() {
        try {
            Connection con = DriverManager.getConnection("jdbc:mysql://localhost:3306/college", "root", "");
            Statement stmt = con.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_READ_ONLY);
            ResultSet rs = stmt.executeQuery("SELECT * FROM Syllabus WHERE branch_name = 'Artificial intelligence and ML';");
